...
from .dataset import IterableImageDataset, ImageDataset, ImageFileList, ImageFileList_Multi, DogAgeImageFileList, DogBodyHeadFileList
...
def create_dataset(
    ...
    if name.startswith('torch/'):
        ...
        ds = ImageFileList(root, split=split, **kwargs)

    elif name == 'file_list_multi':
        ds = ImageFileList_Multi(root, split=split, **kwargs)

    elif name == 'dog_age':
        # from dogAge.dataset import AgeDataset
        # root = "/public/share/challenge_dataset/AFAC-Track3-2023"
        # # _split = 'val' if split == 'validation' else split
        # ds = AgeDataset(
        #     images_path=os.path.join(root, f'{split}set'),
        #     annotations_path=os.path.join(root, 'annotations'),
        #     split=split
            # )

        ds = DogAgeImageFileList(root, split=split, **kwargs)
    elif name == 'dog_bodyhead':
        ds = DogBodyHeadFileList(root, split=split, **kwargs)
    else:
